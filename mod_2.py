...
# closing long & opening short
    ...
    elif tq_close <= df['Adj Close'][i - 1] - (df['Adj Close'][i - 1] * 0.15) and tq_pos == 1 and sq_pos == 0:
        tq_pos = 0
        close_long()
        sq_pos = 1
        opening_short()
...
# closing long (lock in profit)
    elif tq_pos == 1 and sq_pos == 0 and tq_close >= (tq_buys[-1] + (tq_buys[-1] * 0.1)) and tq_close <= eight_ema:
        tq_pos = 0
        close_long()
